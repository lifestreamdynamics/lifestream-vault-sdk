...
async function importKey(keyHex) {
    ...
    return globalThis.crypto.subtle.importKey('raw', keyBytes, { name: ALGORITHM }, false, ['encrypt', 'decrypt']);
}
/**
...
export async function encrypt(plaintext, keyHex) {
    ...
    const encryptedBuffer = await globalThis.crypto.subtle.encrypt({ name: ALGORITHM, iv, tagLength: AUTH_TAG_LENGTH_BYTES * 8 }, key, encoded);
 ...
 */
export async function decrypt(envelopeJson, keyHex) {
    ...
    const key = await globalThis.crypto.subtle.importKey('raw', keyBytes, { name: ALGORITHM }, false, ['encrypt', 'decrypt']);
    ...
    const decryptedBuffer = await globalThis.crypto.subtle.decrypt({ name: ALGORITHM, iv: iv, tagLength: AUTH_TAG_LENGTH_BYTES * 8 }, key, combined);
